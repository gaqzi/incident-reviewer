...
type causeService interface {
	Save(ctx context.Context, cause known.Cause) (known.Cause, error)
...
func KnownCausesHandler(service causeService) func(chi.Router) {
	a := causesHandler{
		htmx:    htmx.New(),
		service: service,
	}
...
func (a *causesHandler) New(w http.ResponseWriter, r *http.Request) {
	...
	layout := a.layout(partial.
		NewID("causes",
			"templates/known-causes/new.html",
			"templates/known-causes/_fields.html",
		),
	)
...
func (a *causesHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Error("failed to save new known cause", "error", err)
		h.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		slog.Error("failed to fetch all known causes after proposing new cause", "error", err)
		h.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	layout := a.layout(partial.
		NewID("causes",
			"templates/known-causes/binding/only-options.html",
			"templates/known-causes/binding/__causes-options.html",
		).
		AddData("SelectedCauseID", cause.ID.String()).
		AddData("KnownCauses", convertKnownCauseToHttpObjects(causes)),
	)

	if err := layout.WriteWithRequest(r.Context(), w, r); err != nil {
		slog.Error("failed to render partial known-causes/binding/", "error", err)
		http.Error(w, "failed to render", http.StatusInternalServerError)
		return
	}
}
